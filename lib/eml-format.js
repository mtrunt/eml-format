  ...
  try {
    ...
    if (typeof data.to != "undefined") {
      if (Array.isArray(data.to)) {
        data.headers["To"] = data.to.map(to => emlformat.toEmailAddress(to)).join(',')
      } else {
    ...
    if (typeof data.cc != "undefined") {
      if (Array.isArray(data.cc)) {
        data.headers["Cc"] = data.cc.map(cc => emlformat.toEmailAddress(cc)).join(',')
      } else {
    ...
    if (typeof data.bcc != "undefined") {
      if (Array.isArray(data.bcc)) {
        data.headers["Bcc"] = data.bcc.map(bcc => emlformat.toEmailAddress(bcc)).join(',')
      } else {
        data.headers["Bcc"] = (typeof data.bcc == "string" ? data.bcc : emlformat.toEmailAddress(data.bcc));
      }
    }
    ...
    if (data.attachments) {
      for (var i = 0; i < data.attachments.length; i++) {
        ...
        if (attachment.encoded) {
          eml += wrap(attachment.data, 76) + EOL;
        }
